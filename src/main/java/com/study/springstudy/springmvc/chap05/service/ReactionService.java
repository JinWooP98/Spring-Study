...
public class ReactionService {
    ...
    // 좋아요 중간처리
    public ReactionDto like(long bno, String account) {
        Reaction reaction = handleReaction(bno, account, ReactionType.LIKE);

        return getReactionDto(bno, reaction);
    }

    private ReactionDto getReactionDto(long bno, Reaction reaction) {
        ...
        return ReactionDto.builder()
                .likeCount(reactionMapper.countLikes(bno))
                .dislikeCount(reactionMapper.countDisLikes(bno))
                .userReaction(reactionType)
                .build();
    }

    // 싫어요 중간처리
    public ReactionDto dislike(long bno, String account) {
        Reaction reaction = handleReaction(bno, account, ReactionType.DISLIKE);
