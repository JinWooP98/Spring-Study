...
public class ScoreSpringJdbcRepository implements ScoreRepository {
    ...
    public boolean save(Score score) {
        ...
        return template.update(sql, score.getStuName(), score.getKor(), score.getEng(),
                score.getMath(), score.getTotal(), score.getAverage(), score.getGrade().toString()) == 1;
    }
    ...
    public boolean updateScore(Score s) {
        String sql = "UPDATE tbl_score " +
                "SET kor = ?, eng = ?, math = ?, average = ?, total = ?, grade = ? " +
                "WHERE stu_num = ?";
        return template.update(sql, s.getKor(), s.getEng(), s.getMath(), s.getAverage(), s.getTotal(), s.getGrade().toString()
        ,s.getStuNum()) == 1;
    }
}
